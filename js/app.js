...
function checkValues() {
  ...
  if (width > 400 || height > 100 || depth > 400) {
    // show an error message
    alert("Values cannot be greater than 400cm (width,depth) or 100cm (height), please try again.");
    return false;
  } else {
...
const form = document.getElementById("product-form");
  ...
  form.addEventListener("submit", (event) => {
    ...
    totalPriceField.value = calculatePrice();
  });
  ...
  // check if any dimension is over 400 / 100
	if (width > 400 || height > 100 || depth > 400) {
	  // show an error message
	  alert("The price has not been updated! values cannot be greater than 400cm (width,depth) or 100cm (height), please try again.");
	  return false;
	}
  ...
  const totalPrice = (basePrice + widthPrice + heightPrice + depthPrice) * quantity;
...
function addToCart() {
  ...
  let data = JSON.stringify(cartItem);
  xhr.send(data);
